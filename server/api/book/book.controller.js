...
export function index(req, res) {
  return Book.find().limit(16).exec()
    .then(respondWithResult(res))
    .catch(handleError(res));
}

export function search(req, res) {
  console.log('search:',req.params.book);
  return Book.find({"Title":{'$regex':req.params.book,'$options':'i'}}).limit(16).exec()
    .then(respondWithResult(res))
    .catch(handleError(res));
}
...
export function showpublisher(req, res) {
  return Book.find({"Source":req.params.book}).limit(52).exec()
    .then(handleEntityNotFound(res))
    .then(respondWithResult(res))
    .catch(handleError(res));
}
